...
const taskModule = (function () {
  ...
  function getTasks(skip = 0, top = 10, filterConfig = {}) {
    ...
    if (filterConfig.description) {
      filteredTasks = filteredTasks.filter((task) => task.description
    ...
    if (filterConfig.status) {
      filteredTasks = filteredTasks.filter((task) => task.status === filterConfig.status);
    }

    if (filterConfig.priority) {
      filteredTasks = filteredTasks.filter((task) => task.priority === filterConfig.priority);
    }
  ...
  function addTask(name, description, priority, assignee = user, status = 'To Do', isPrivate = false) {
    ...
    const id = String(getMaxIdTasks() + 1);
